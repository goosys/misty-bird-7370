...
Administrate::Page::Base.prepend (
  Module.new {
    ...
    def resource_name
      @resource_name ||=
        ActiveSupport::Inflector.demodulize(dashboard.class.to_s).chomp(DASHBOARD_SUFFIX).underscore
    end
  }
)
...
Administrate::BaseDashboard.singleton_class.prepend (
  Module.new {
    ...
    def model
      ActiveSupport::Inflector.demodulize(to_s).chomp(DASHBOARD_SUFFIX).classify.constantize
    end
  }
)
